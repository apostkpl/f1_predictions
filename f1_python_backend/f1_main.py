...
# I'll need those to be used afterwards, outside of main
full_df = None
X_train = None
y_train = None
X_future = None
...
# Ask the user if they want cached data or new data from FastF1 API:
cached = input("Use cached data? Yes/No: ")
cach = (True if cached.lower() == "yes" else False)
fastf1.Cache.offline_mode(cach)
...
def load_and_process_data():
    # Use the global scope variables
    global full_df, X_train, y_train, X_future, ids, train_ids
    ...
    # Re-seperate the data for cleaning na values (only on the past results, we dont wanna drop any future rows)
    historical_data = full_df[full_df['isPredictionData'] != 1].copy()
    future_data = full_df[full_df['isPredictionData'] == 1].copy()

    # Remove any Null values
    drop_na(historical_data)

    # Recreate once more the full_df
    full_df = pd.concat([historical_data, future_data], ignore_index = True)
